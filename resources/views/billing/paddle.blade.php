        ...
        <div class="bg-white dark:bg-slate-800 shadow-xl rounded-xl w-full max-w-md p-8 border border-slate-200/50 dark:border-slate-700/50 relative overflow-hidden">
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between rounded-md border p-4">
                    ...
                    <div class="text-right">
                        ...
                        <div class="font-semibold">
                            <div>{{ 'KES' }} {{ number_format($ui['plan']['display_price_kes'] ?? ($ui['plan']['price_kes'] ?? 0), 0) }}</div>
                            <div class="text-xs text-muted-foreground">{{ 'USD' }} {{ number_format($ui['plan']['display_price_usd'] ?? ($ui['plan']['price_usd'] ?? 0), 2) }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    @else
                        <div class="text-sm text-muted-foreground">Checkout is not available at the moment. Please try again later.</div>
                    @endif
                </div>

                <div class="text-center">
                    <a href="{{ $ui['return_url'] ?? route('billing.index') }}" class="inline-block text-sm text-muted-foreground hover:underline">Back to Billing</a>
                </div>
            </div>
        </div>
    </div>
