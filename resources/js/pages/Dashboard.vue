...
const setupTrendChart = () => {
    ...
    const textColorSecondary = documentStyle.getPropertyValue('--color-muted-foreground');
...
const weeklyTrend = computed(() => {
    const diff = props.stats.recent_feedback - props.stats.previous_week_feedback;
    if (diff > 0) return { icon: TrendingUp, color: 'text-green-600', text: `+${diff} from last week` };
    if (diff < 0) return { icon: TrendingDown, color: 'text-red-600', text: `${diff} from last week` };
    return { icon: TrendingUp, color: 'text-muted-foreground', text: 'No change' };
});

const monthlyTrend = computed(() => {
    const diff = props.stats.current_month_feedback - props.stats.previous_month_feedback;
    if (diff > 0) return { icon: TrendingUp, color: 'text-green-600', text: `+${diff} from last month` };
        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
            ...
            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                ...
                <Card>
                    ...
                    <CardContent>
                        <div class="text-2xl font-bold">{{ stats.total_feedback }}</div>
                        <div class="flex items-center gap-1 text-xs text-muted-foreground mt-1">
                            <component :is="weeklyTrend.icon" :class="['h-3 w-3', weeklyTrend.color]" />
                ...
                <Card>
                    ...
                    <CardContent>
                        <div class="text-2xl font-bold">{{ stats.average_rating }}</div>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div class="text-2xl font-bold">{{ stats.response_rate }}%</div>
                ...
                <Card>
                    ...
                    <CardContent>
                        ...
                        <div class="flex items-center gap-1 text-xs text-muted-foreground mt-1">
                            <component :is="monthlyTrend.icon" :class="['h-3 w-3', monthlyTrend.color]" />
            ...
            <div class="grid gap-4 md:grid-cols-3">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Download or view your QR code</CardDescription>
                    </CardHeader>
                    <CardContent class="space-y-3">
                        <div v-if="quick_links.qr_code_path" class="flex justify-center p-4 bg-muted rounded-lg">
                            <img 
                        ...
                        <div v-else class="flex justify-center p-8 bg-muted rounded-lg">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Share this link for reviews</CardDescription>
                    </CardHeader>
                    <CardContent class="space-y-3">
                        ...
                        <div class="flex gap-2">
                            <Button 
                                variant="outline" 
                                class="flex-1 gap-2"
                                @click="copyToClipboard(quick_links.review_url, 'Review link')"
                            >
                                <Copy class="h-4 w-4" />
                                Copy Link
                            </Button>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Your public feedback page</CardDescription>
                    </CardHeader>
                    <CardContent class="space-y-3">
                        <div class="p-3 bg-muted rounded-lg text-sm font-mono break-all">
                        ...
                        <div class="flex gap-2">
                            <Button 
                                variant="outline" 
                                class="flex-1 gap-2"
                                @click="copyToClipboard(quick_links.public_profile_url, 'Public profile link')"
                            >
                                <Copy class="h-4 w-4" />
                                Copy Link
                            </Button>
                            <Button 
                                variant="outline"
                                @click="openInNewTab(quick_links.public_profile_url)"
                            >
                                <ExternalLink class="h-4 w-4" />
                            </Button>
                        </div>
            ...
            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                <Card class="lg:col-span-2">
                    ...
                    <CardContent>
                        <Chart type="line" :data="trendChartData" :options="trendChartOptions" class="h-[300px]" />
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent class="flex justify-center items-center">
                        <Chart type="doughnut" :data="sentimentChartData" :options="sentimentChartOptions" class="w-full h-[300px]" />
                    </CardContent>
                </Card>
            </div>
            ...
            <div class="grid gap-4 lg:grid-cols-3">
                ...
                <Card>
                    ...
                    <CardContent class="flex justify-center items-center">
                        <Chart type="doughnut" :data="ratingChartData" :options="ratingChartOptions" class="w-full h-[300px]" />
                    </CardContent>
                </Card>
                    ...
                    <CardContent>
                        ...
                        >
                            <Column field="customer_name" header="Customer" style="min-width: 150px">
                                <template #body="{ data }">
                                    <div>
                                        ...
                                        <div class="text-xs text-muted-foreground">{{ data.customer_email }}</div>
                                    </div>
                                </template>
                            </Column>
                            <Column field="rating" header="Rating" style="min-width: 120px">
                                <template #body="{ data }">
                                    <Rating :modelValue="data.rating" :cancel="false" readonly class="text-sm" />
                                </template>
                            </Column>
                            <Column field="comment" header="Comment" style="min-width: 200px">
                                <template #body="{ data }">
                                    <div class="line-clamp-2">{{ data.comment || 'No comment' }}</div>
                                </template>
                            </Column>
                            ...
                            <Column field="submitted_at" header="Submitted" style="min-width: 120px">
                                <template #body="{ data }">
                                    <span class="text-xs">{{ data.submitted_at }}</span>
                                </template>
                            </Column>
                        </DataTable>
                    </CardContent>
